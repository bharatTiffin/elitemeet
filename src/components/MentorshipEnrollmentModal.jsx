...
function MentorshipEnrollmentModal({ program, onClose, onEnrollmentSuccess }) {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const options = {
        ...
        name: 'Elite Meet',
        description: '6-Month Full Mentor Guidance Program',
        order_id: order.id,
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 border border-white/20 rounded-3xl p-8 max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400">Transform your preparation journey</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex gap-4">
          <button
            onClick={onClose}
            className="flex-1 px-6 py-3 bg-gray-700/50 hover:bg-gray-600/50 text-gray-300 rounded-xl font-semibold transition-all duration-300 border border-gray-600/50"
          >
            Cancel
          </button>
          <button
